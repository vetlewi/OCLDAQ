...
void sort_singles(std::vector<word_t> buffer)
{
    ...
    for (auto & buf : buffer){
        ...
        dinfo = GetDetector(buf.address);

        switch (dinfo.type) {
            case labr:
                spec_fill(LABRSP_ID, buf.adcdata, dinfo.detectorNum);
                spec_fill(LABRCSP_ID, gain_labr[dinfo.detectorNum]*buf.adcdata + shift_labr[dinfo.detectorNum], dinfo.detectorNum);
                break;
            case deDet:
                spec_fill(DESP_ID, buf.adcdata, dinfo.detectorNum + 8*dinfo.telNum);
                break;
            case eDet:
                spec_fill(ESP_ID, buf.adcdata, dinfo.detectorNum);
                break;
            case ppac:
                spec_fill(PPAC_ID, buf.adcdata, dinfo.detectorNum);
                break;
            default:
                break;
        }
