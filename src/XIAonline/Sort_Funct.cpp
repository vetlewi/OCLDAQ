...
void sort_singles(const std::vector<word_t> &buffer)
{
    ...
    for (const auto & i : buffer){
        ...
        case labr:
            ...
            spec_fill(LABRCSP_ID, gain_labr[dinfo.detectorNum]*i.adcdata + shift_labr[dinfo.detectorNum], dinfo.detectorNum);
            if ( i.cfdfail )
                spec_fill(LABRCFD_ID, i.adcdata, dinfo.detectorNum);
            break;
        case deDet:
            spec_fill(DESP_ID, i.adcdata, dinfo.detectorNum + 8*dinfo.telNum);
            if ( i.cfdfail )
                spec_fill(DECFD_ID, i.adcdata, dinfo.detectorNum + 8*dinfo.telNum);
            break;
        case eDet:
            spec_fill(ESP_ID, i.adcdata, dinfo.detectorNum);
            if ( i.cfdfail )
                spec_fill(ECFD_ID, i.adcdata, dinfo.detectorNum);
            break;
        case eGuard:
            spec_fill(GUARD_ID, i.adcdata, dinfo.detectorNum);
            break;
        case ppac:
            spec_fill(PPAC_ID, i.adcdata, dinfo.detectorNum);
            break;
        default:
            break;
        }
